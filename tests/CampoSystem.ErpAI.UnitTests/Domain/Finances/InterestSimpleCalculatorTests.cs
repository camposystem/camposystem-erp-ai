...
public class InterestSimpleCalculatorTests
{
    [Theory]
    [InlineData(100, 10, 2, TimeUnit.Months, 120)]
    [InlineData(100, 25, 2, TimeUnit.Months, 150)]
    [InlineData(100, 50, 2, TimeUnit.Months, 200)]
    [InlineData(100, 150, 2, TimeUnit.Months, 400)]
    public void Given_A_Money_Positive_Interest_Rate_And_Period_Should_Calculate_Simple_Interest(int capital, int percentage, int period, TimeUnit timeUnit, int expected)
    {   // Arrange
        var moneyCapital = Money.From((decimal)capital);
        IInterestCalculator calculator = new InterestSimpleCalculator();
    ...
    [Theory]
    [InlineData(100.0, 12.0, RateType.Nominal, TimeUnit.Years, 1.0, TimeUnit.Months, 101.0)]
    public void Given_A_Money_Positive_Interest_Rate_With_A_Different_Period_Should_Calculate_Simple_Interest(
